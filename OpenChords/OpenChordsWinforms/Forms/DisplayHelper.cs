...
namespace OpenChords.Forms
{
	...
	partial class DisplayForm
	{
		...
		/// <summary>
		/// create the constructor for the example displayform
		/// </summary>
		/// <param name="song"></param>
		/// <param name="width"></param>
		/// <param name="height"></param>
		private DisplayForm(Song song, int width, int height, DisplayAndPrintSettings settings)
		{
			//
			...
			//this.Location = new Point(width, SystemInformation.WorkingArea.Height/2 - this.Height/2);
        ...
        //public static DisplayForm createExampleDisplayForm(DisplaySettingsForm owner, Song song, OpenChordsDisplayAndPrintSettings exampleSongSettings)
        //{
        //    //DisplaySettings.loadSettings();
			
			
        //    DisplayForm form = new DisplayForm(song, SystemInformation.WorkingArea.Width/2, SystemInformation.WorkingArea.Height/2, exampleSongSettings);
        //    form.Location = new Point(owner.Width, 0);
        ...
        //        public static DisplayForm createExamplePage(DisplaySettingsForm owner, Song song, OpenChordsDisplayAndPrintSettings exampleSongSettings)
        //{
        //    //DisplaySettings.loadSettings();
			
			
        //    int width = (int)(SystemInformation.WorkingArea.Height * (210.0/297));
			
        //    DisplayForm form = new DisplayForm(song, width, SystemInformation.WorkingArea.Height, exampleSongSettings);
        //    form.Location = new Point(owner.Width, 0);
        ...
        void drawPartialVerseContents(SongVerse songVerse, bool firstHalfOfPage)
		{
            ...
            graphicsObj.DrawString("...Press space to continue to next page...",
			                       lyricsFormattor.Font,
			                       lyricsFormattor.Brush,
			                       leftPosition,
			                       heightPosition);
		...
		{
			if (indexOfSongIndices - 1 >= 0)
			{
				songPageIndices.RemoveAt(songPageIndices.Count - 1);
				indexOfSongIndices--;
		...
		void gotoNextPage()
		{
			//update songPageIndices variable
			int endOfPreviousPage = songPageIndices[songPageIndices.Count - 1];
			if (versesDisplayed > 0)
			{
				songPageIndices.Add(endOfPreviousPage + versesDisplayed);
			}
			
			if (indexOfSongIndices + 1 < songPageIndices.Count)
			{
				indexOfSongIndices++;
				Invalidate(); //force redraw
			}
		...
		void gotoNextSong()
		{
			if (setIndex +1 < currentSet.songList.Count)
			{
				setIndex++;
		...
		void gotoSong(int songNumber)
		{
			if (songNumber >= 0 && songNumber < currentSet.songList.Count)
			{
				setIndex = songNumber;
				initialiseSongIndicesList();
				Invalidate(); //force redraw
